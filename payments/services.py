...
def create_payment(user, order, card_data):
    ...
    total_items_price = sum(
        Decimal(item.price) * item.quantity for item in order.items.all()
    )

    discount_ratio = (
        (discount_amount / total_items_price) if total_items_price > 0 else Decimal("0")
    )
    ...
    for item in order.items.all():
        ...
        discount_price = (original_price * (1 - discount_ratio)).quantize(
            Decimal("0.01")
        )
    ...
    if basket_total != order_total:
        raise ValidationError(
            f"Basket total ({basket_total}) does not match order total ({order_total})"
        )
    ...
    buyer = {
        ...
        "registrationAddress": (
            order.delivery_address.address_line if order.delivery_address else ""
        ),
        "ip": "85.34.78.112",
        "city": (
            order.delivery_address.city.name
            if order.delivery_address and order.delivery_address.city
            else ""
        ),
        "country": "Turkey",
    }
    ...
    billingAddress = {
        ...
        "city": (
            ...
            if order.billing_address and order.billing_address.city
            else ""
        ),
        "country": "Turkey",
    ...
    deliveryAddress = {
        "contactName": (
            order.delivery_address.full_name if order.delivery_address else ""
        ),
        "city": (
            order.delivery_address.city.name
            if order.delivery_address and order.delivery_address.city
