...
def create_payment(user, order, cart_data):

    if order.total_price <= 0:
        raise ValidationError(
            {"error": "Order total price must be greater than zero to process payment."}
        )
    ...
    payment_card = {
        "cardHolderName": cart_data["cardHolderName"],
        "cardNumber": cart_data["cardNumber"],
        "expireMonth": cart_data["expireMonth"],
        "expireYear": cart_data["expireYear"],
        "cvc": cart_data["cvc"],
        "registerCard": "0",
    }
